...
int kepler(J, e, rect, polar) double J, rect[], polar[]; struct orbit *e; { // declaration fonction
  ...
  if( e->ptable ) {
    if( e == &earth ) g3plan (J, e->ptable, polar, 3);  // appel fonction
    else              gplan  (J, e->ptable, polar);      // appel fonction
  ...
  */
  ...
  else       alat = W + DTR*argperih; /* mean longitude not given */
  ...
  precess( rect, e->equinox, 1 );  // appel fonction

  /* If earth, adjust from earth-moon barycenter to earth
   * by AA page E2.
   */

  if( e == &earth ) embofs( J, rect, &r ); /* see below */  // appel fonction

  /* Rotate back into the ecliptic.  */
  
  epsiln( J2000 );  // appel fonction
...
int embofs( J, ea, pr ) double J; double ea[]; double *pr; { // declaration fonction
  ...
  gmoon( J, pm, polm ); // appel fonction

  /* Precess the lunar position
   * to ecliptic and equinox of J2000.0
   */
  
  precess( pm, J, 1 ); // appel fonction
