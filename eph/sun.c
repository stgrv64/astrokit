...
int dosun(TOPOCENTRIC *infos) { // declaration fonction
  ...
  for( i=0; i<2; i++ ) {
    ...
    kepler( TDT-t, &earth, ecr, pol ); // appel fonction
  }
  r = pol[2];
  ...
  deltap( ecr, rec, &dradt, &ddecdt );  /* see dms.c */  // appel fonction
  ...
  precess( ecr, TDT, -1 );  // appel fonction

  for( i=0; i<3; i++ ) rec[i] = ecr[i];

  /* Nutation. */
  
  epsiln( TDT );          // appel fonction
  nutate( TDT, ecr );     // appel fonction
...
infos->dec=ecr[2] ;
  ...
  altaz( infos, pol, UT );   // appel fonction
