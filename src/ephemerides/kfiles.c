 ...
 */
double tlong = -0.39;	/* Cambridge, Massachusetts */
double tlat = 43.3; /* geocentric */
double glat = 43.3; /* geodetic */
double height = 0 ;
...
int kinit() // declaration fonction
{
   ...
   * AA page K5 */
    ...
    if( glat < 0.0 ) tlat = -tlat;
    trho /= aearth;

    /* Reduction from geodetic latitude to geocentric latitude * AA page K5 */
   /*
    tlat = glat - 0.19242861 * sin(2.0*u) + 0.00032314 * sin(4.0*u) - 0.00000072 * sin(6.0*u);
...
int getorbit(el) struct orbit *el; {
  ...
  sscanf( s1, "%lf %lf %lf %lf %lf %lf",&el->epoch, &el->i, &el->W, &el->w, &el->a, &el->dm );
  sscanf( s2, "%lf %lf %lf %lf %lf %15s", &el->ecc, &el->M,&el->equinox, &el->mag, &el->sdiam, &el->obname[0] );
  ...
  if( strcmp( &el->obname[0], "Earth" )  ) return(0); 
  else {
    u = (char *)&earth;
    v = (char *)el;
...
int getstar(el) struct star *el; {
  ...
  if( x == 2000.0 )      x = J2000;
  else if( x == 1950.0 ) x = B1950;
  else if( x == 1900.0 ) x = J1900;
  else                   x = J2000  +  365.25 * (x - 2000.0);
  ...
  if( z < 1.0 ) {
     if( z <= 0.0 ) el->px = 0.0;
     else           el->px = STR * z;  /* assume px in arc seconds */
  } else {
